...
Another system implemented this year was the remote telemetry data acquisition system. This would be done by a Raspberry PI. Separate from the STM32, this computing module read non operational critical data into a buffer for the remote server to read and display. The goal was to give both the driver and ground station information about various diagnostics such as battery temperatures, speed readings, charge, and various other information. For operation critical data that gets read first by the STM32, a communication channel should be set up between the STM32 and PI for the PI to read from the STM32.
